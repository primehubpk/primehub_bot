...
export async function generateSalaarReply(customerMessage:string,products:Product[],adminNotes=""){
 ...
 const notes=adminNotes.trim()?`Latest admin What's new notes (treat as current PrimeHubMaal truth): ${adminNotes.trim()}`:"No latest admin notes.";
 ...
 try{if(openaiKey){const r=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${openaiKey}`},body:JSON.stringify({model:process.env.OPENAI_MODEL||"gpt-4.1-mini",temperature:0.4,max_tokens:180,messages:[{role:"system",content:system},{role:"user",content:`Customer: ${customerMessage}\nCatalog tool results:\n${productText}`}]})});if(!r.ok)throw new Error(`OpenAI ${r.status}`);const data=await r.json();return{text:(data?.choices?.[0]?.message?.content||"Ji, batayein kis cheez mein help chahiye?").trim(),configured:true};}
 const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":anthropicKey!,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:process.env.ANTHROPIC_MODEL||"claude-3-5-haiku-latest",max_tokens:180,temperature:0.4,system,messages:[{role:"user",content:`Customer: ${customerMessage}\nCatalog tool results:\n${productText}`}]})});if(!r.ok)throw new Error(`Anthropic ${r.status}`);const data=await r.json();return{text:(data?.content?.[0]?.text||"Ji, batayein kis cheez mein help chahiye?").trim(),configured:true};
